...
@section('title', 'Peminjaman Lapangan')

@section('content')
    <div class="flex justify-between items-center mb-6">
        <h1 class="text-2xl font-bold">Peminjaman Lapangan</h1>
        <a href="/mahasiswa/lapangan/create" class="bg-blue-600 text-white px-4 py-2 rounded hover:bg-blue-700">
            + Ajukan Peminjaman
        </a>
    </div>
        ...
        <table class="w-full text-left">
            <thead class="bg-gray-50">
                <tr>
                    ...
                    <th class="px-4 py-3 text-sm font-medium text-gray-700">Lapangan</th>
            ...
            <tbody>
                    ...
                    <tr class="border-t">
                        ...
                        <td class="px-4 py-3">
                            @if($p->tipe_pinjam === 'per_hari')
                                {{ $p->tanggal_mulai }} <br><span class="text-xs text-gray-500">s/d</span><br>
                                {{ $p->tanggal_selesai }}
                            @else
                                {{ $p->tanggal_mulai }}
                            @endif
                        ...
                        <td class="px-4 py-3">
                            {{ $p->tipe_pinjam === 'per_jam' ? ($p->jam_mulai . ' - ' . $p->jam_selesai) : 'Full Day' }}</td>
                        ...
                        <td class="px-4 py-3">
                            @if($p->status === 'pending')
                                <form action="/mahasiswa/lapangan/{{ $p->id_peminjaman }}/cancel" method="POST" class="inline"
                ...
                @empty
                    <tr>
                        <td colspan="7" class="px-4 py-6 text-center text-gray-500">Belum ada peminjaman lapangan.</td>
                    </tr>
                @endforelse
            </tbody>
        </table>
